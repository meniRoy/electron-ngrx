...
export class WindowCommunicationService {
  ...
  constructor(private router: Router) {
    ...
    this.replay = fromEvent(this.ipcRenderer, communicationChannel.replay).pipe(map(this.extractDataFromEvent), share());
    this.subscription = fromEvent(this.ipcRenderer, communicationChannel.subscription).pipe(map(this.extractDataFromEvent));
  }
  ...
  sendToId<T, R>(windowId: number, data: T): Observable<R> {
    const messageId = this.generateMessageId();
    this.sendToWindow(communicationChannel.id, windowId, {data, senderId: this.myId, messageId});
    return this.getReplayByMessageId<R>(messageId);
  }

  listenToSubscriptionRequest<T>() {
    return this.subscription.pipe(map((message: SubscriptionRequest<T>) => ({
      data: message.data,
      response: (observable: Observable<any>) => {
        const channel = communicationChannel.subscription + message.messageId;
        const cleanup = this.onWindowClose(this.remote.BrowserWindow.fromId(message.senderId));
        this.sendObservableOnChannel(channel, cleanup, observable, message.senderId);
        this.sendToWindow(channel, message.senderId, {type: SubscriptionCommand.listening});
      }
    })), share());
  }

  private sendObservableOnChannel(channel: string, cleanup: Observable<void>, srcObservable: Observable<any>, windowId: number) {
    let subscription: Subscription;
    fromEvent(this.ipcRenderer, channel).pipe(map(this.extractDataFromEvent), takeUntil(cleanup))
      .subscribe((command: { type: SubscriptionCommand }) => {
        if (command.type === SubscriptionCommand.subscribe) {
          subscription = srcObservable.pipe(takeUntil(cleanup)).subscribe(
            (data) => this.sendToWindow(channel, windowId, {type: SubscriptionCommand.next, data}),
            (error) => this.sendToWindow(channel, windowId, {type: SubscriptionCommand.error, data: error}),
            () => this.sendToWindow(channel, windowId, {type: SubscriptionCommand.complete})
          );
  ...
  private subscribeToWindow<T, R>(window: BrowserWindow, data: T): Observable<R> {
    ...
    window.webContents.send(communicationChannel.subscription, {data, messageId, senderId: this.myId});
    return from(waitForWindowToListen).pipe(switchMap(this.chanelSubscriptionToObservable<R>(window, channel)), share());
  }


  private chanelSubscriptionToObservable<T>(window: BrowserWindow, channel: string): () => Observable<T> {
    return () => new Observable((subscriber) => {
      ...
      window.webContents.send(channel, {type: SubscriptionCommand.subscribe});
      return () => {
    ...
    return new Promise<void>((resolve, reject) => {
      this.ipcRenderer.once(channel, (event, data: { type: string }) => {
        data.type === SubscriptionCommand.listening ?
          ...
          reject(`The first message on channel is supposed to be:{type:${SubscriptionCommand.listening}} but the message received was: ${JSON.stringify(data)}  please open issue in our github repository`);
      });
    });
  }
}
